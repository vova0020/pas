...
const Stanok19Additive = ({znpData, stanckiSpisock, onChangeStanock}) => {
    return (
        <select className={"addressPallet19"}onChange={(eventTarget, eventId, znpDatas) => {
            onChangeStanock(eventTarget.target , znpData)
        }}>
            {znpData.machine_pallet_additive_19 != null
                ? (<option value="standart">{znpData.machine_pallet_additive_19}✔</option>)
...
export default React.memo(Stanok19Additive);