...
const Stanok20Additive = ({znpData, stanckiSpisock, onChangeStanock}) => {
    return (
        <select className={"addressPallet20"} onChange={(eventTarget, eventId, znpDatas) => {
            onChangeStanock(eventTarget.target , znpData)
        }}>
            {znpData.machine_pallet_additive_20 != null
                ? (<option value="standart">{znpData.machine_pallet_additive_20}✔</option>)
...
export default React.memo(Stanok20Additive);